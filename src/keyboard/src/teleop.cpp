...
void runMission(){
  ...
  if(missions.guide && line.possible){
    ...
    else if (missions.buoy_red && buoy.possible_red){

    teleop.sidemove_input=buoy.x_offset_red;
    teleop.depth_setpoint=DEPTH_BUOY;
  }

  else if (missions.buoy_green && buoy.possible_green){

    teleop.sidemove_input=buoy.x_offset_green;
    teleop.depth_setpoint=DEPTH_BUOY;
  }

 else if(inBucket && bucket.possible ){
  ...
  else if(missions.gate && gate.possible){
