...
class Buoys:
 ...
 maxRGB =
            (np.max(outR), np.max(outG), np.max(outB))
 ...
 outB =
            255*outB/factor
 outImg = cv2.merge((np.uint8(outB), np.uint8(outG),
            np.uint8(outR)))
 '''
 ...
 cv2.putText(self.image,
            str(fin), (30, 30),cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255))
 #print
            ("final",fin)
